...
class Weibo(object):
    def __init__(self, out_user, config):
        """Weibo类初始化"""
        ...
        user_id_list = out_user  # config['user_id_list']
        ...
        if not isinstance(user_id_list, list):
            ...
            user_config_list = [{'user_id': user_id, 'since_date': self.since_date, 'query_list': query_list} for user_id in user_id_list]
    ...
    def validate_config(self, config):
        ...
        argument_list = ['filter', 'original_pic_download', 'retweet_pic_download', 'original_video_download', 'retweet_video_download', 'download_comment']
    ...
    def get_weibo_json(self, page):
        """获取网页中微博json数据"""
        params = {'container_ext': 'profile_uid:' + str(self.user_config['user_id']), 'containerid': '100103type=401&q=' + self.query, 'page_type': 'searchall'} if self.query else {'containerid': '107603' + str(self.user_config['user_id'])}
        params['page'] = page
    ...
    def user_to_csv(self):
        ...
        result_headers = ['用户id', '昵称', '性别', '生日', '所在地', '学习经历', '公司', '注册时间', '阳光信用', '微博数', '粉丝数', '关注数', '简介', '主页', '头像', '高清头像', '微博等级', '会员等级', '是否认证', '认证类型', '认证信息', '上次记录微博信息']
        result_data = [[v.encode('utf-8') if 'unicode' in str(type(v)) else v for v in self.user.values()]]
    ...
    def user_to_mysql(self):
        """将爬取的用户信息写入MySQL数据库"""
        mysql_config = {'host': 'localhost', 'port': 3306, 'user': 'root', 'password': '123456', 'charset': 'utf8mb4'}
        # 创建'weibo'数据库
    ...
    def get_user_info(self):
        ...
        if status_code != 200:
            ...
            raise gotbanError(u"被ban了，需要等待一段时间")
            sys.exit()
        if js['ok']:
            ...
            params = {'containerid': '230283' + str(self.user_config['user_id']) + '_-_INFO'}
            zh_list = [u'生日', u'所在地', u'小学', u'初中', u'高中', u'大学', u'公司', u'注册时间', u'阳光信用']
            en_list = ['birthday', 'location', 'education', 'education', 'education', 'education', 'company', 'registration_time', 'sunshine']
            for i in en_list:
                user_info[i] = ''
            js, _ = self.get_json(params)
            if js['ok']:
                ...
                if isinstance(cards, list) and len(cards) > 1:
                    ...
                    for card in card_list:
                        if card.get('item_name') in zh_list:
                            user_info[en_list[zh_list.index(card.get('item_name'))]] = card.get('item_content', '')
    ...
    def get_video_url(self, weibo_info):
        ...
        if weibo_info.get('page_info'):
            if ((weibo_info['page_info'].get('urls') or weibo_info['page_info'].get('media_info')) and weibo_info['page_info'].get('type') == 'video'):
    ...
    def standardize_info(self, weibo):
        ...
        # -------------------- check weibo
        '''with open('./weibo_exp.txt', 'w') as f:
            wb_str = str(weibo)
            f.write(wb_str)'''
        # ----------------------
    ...
    def _get_weibo_comments_cookie(self, weibo, cur_count, max_count, max_id, on_downloaded):
        ...
        self._get_weibo_comments_cookie(weibo, cur_count, max_count, max_id, on_downloaded)

    def _get_weibo_comments_nocookie(self, weibo, cur_count, max_count, page, on_downloaded):
        """
        :weibo standardlized weibo
        :cur_count  已经下载的评论数
        :max_count 最大允许下载数
        ...
        if cur_count >= max_count:
            return
        id = weibo['id']
        url = "https://m.weibo.cn/api/comments/show?id={id}&page={page}".format(id=id, page=page)
        ...
        try:
            ...
            #没有cookie会抓取失败
            logger.info(u'未能抓取评论 微博id:{id} 内容{text}'.format(id=id, text=weibo['text']))
            return
        ...
        if page >= req_page:
            return
        self._get_weibo_comments_nocookie(weibo, cur_count, max_count, page, on_downloaded)
    ...
    def get_one_page(self, page):
        ...
        try:
            ...
            if js['ok']:
                    ...
                    if w['card_type'] == 9:
                        ...
                        if wb:
                            ...
                            if const.MODE == 'append':
                                ...
                                since_date = datetime.strptime(convert_to_days_ago(self.last_weibo_date, 1), '%Y-%m-%d')
                            if created_at < since_date:
                                ...
                                # 如果要检查还没有检查cookie，不能直接跳出
                                    ...
                                    logger.info(u'{}已获取{}({})的第{}页{}微博{}'.format('-' * 30, self.user['screen_name'], self.user['id'], page, '包含"' + self.query + '"的' if self.query else '', '-' * 30))
                                    return True
                ...
                return True
            logger.info(u'{}已获取{}({})的第{}页微博{}'.format('-' * 30, self.user['screen_name'], self.user['id'], page, '-' * 30))
    ...
    def get_result_headers(self):
        """获取要写入结果文件的表头"""
        result_headers = ['id', 'bid', '正文', '头条文章url', '原始图片url', '视频url', '位置', '日期', '工具', '点赞数', '评论数', '转发数', '话题', '@用户']
        if not self.filter:
    ...
    def parse_sqlite_comment(self, comment, weibo):
        ...
        self._try_get_value('user_avatar_url', 'avatar_hd', sqlite_comment, comment['user'])
    ...
    def update_user_config_file(self, user_config_file_path):
        ...
        with open(user_config_file_path, 'rb') as f:
            try:
                ...
                logger.error(u'%s文件应为utf-8编码，请先将文件编码转为utf-8再运行程序', user_config_file_path)
                sys.exit()
...
def get_config():
    ...
    if not os.path.isfile(config_path):
        logger.warning(u'当前路径：%s 不存在配置文件config.json', (os.path.split(os.path.realpath(__file__))[0] + os.sep))
        sys.exit()
    try:
        ...
        logger.error(u'config.json 格式不正确，请参考 ' u'https://github.com/dataabc/weibo-crawler#3程序设置')
        sys.exit()
